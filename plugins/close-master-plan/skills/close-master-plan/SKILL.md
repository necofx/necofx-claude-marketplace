...
for `closed`, the PR field from whatever the user supplied in Step 4 (or `tasks.md`'s existing `PR`
column) or `—`, and the `rules:` field from the file list Step 6 recorded, or `—` if it approved
nothing.
...
Always stage explicit paths — `docs/plans .claude/rules` — never `git add -A`; the close commit must
...
There's no reopen command because a reopen would have to honestly reverse three different kinds of
change at once: restore a `.claude/rules/*.md` edit the user already approved as a standalone
improvement (not scoped to this plan — reverting it may take back something unrelated), restore
whichever header the stamp replaced (only recoverable if that prior header is still visible in git
history), and undo the `tasks.md` reconciliation the user built by hand in Step 4. Automating that
